...
bool App::OnInit() {
	...
	//// 頂点バッファの生成
	{
		...
		std::cout << "First VertexNum : " << m_meshes[0].Vertices.size() << std::endl;
	}
	...
	// インデックスバッファの生成
	{
		...
		std::cout << "First IndexNum : " << m_meshes[0].Indices.size() << std::endl;
	}
...
void App::Update() {
	...
	//m_CBV[m_FrameIndex].pBuffer->World *= DirectX::XMMatrixRotationY(0.01f);
}
...
void App::Render() {
	...
	// 描画処理
	{
		ID3D12DescriptorHeap* heap = m_DespManager.GetHeapCBV_SRV_UAV();
		...
		//m_pCmdList->IASetPrimitiveTopology(D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
		//m_pCmdList->IASetVertexBuffers(0, 1, &m_VBV);                                                     /* 頂点のセット */
		//m_pCmdList->IASetIndexBuffer(&m_IBV);                                                             /* インデックスのセット */
		//m_pCmdList->SetGraphicsRootConstantBufferView(0, m_CBV[0].CBVDesc.BufferLocation); /* 変換行列のセット */
		//m_pCmdList->SetGraphicsRootConstantBufferView(1, m_LightCBV.CBVDesc.BufferLocation);                 /* ライトをセット */
		//m_pCmdList->SetGraphicsRootConstantBufferView(2, m_Material.CBVDesc.BufferLocation);              /* マテリアルのセット */
		//m_pCmdList->SetGraphicsRootDescriptorTable(3, m_Texture.HandleGPU);                    /* ディフューズマップのセット */
		//m_pCmdList->SetGraphicsRootDescriptorTable(4, NormalMapSRV.HandleGPU);                     /* 法線マップのセット */
		//
