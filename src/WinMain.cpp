...
bool App::OnInit() {
	...
	//	//	std::cout << "position:(" << vertices[i].position.x << "," << vertices[i].position.y << ") TexCoord:(" << vertices[i].textureCoordinate.x << "," << vertices[i].textureCoordinate.y << ")" << std::endl;
	//	//}
	...
	//	// マッピングされた頂点バッファの内容を出力
	//	//uint32_t* mappedVertices = reinterpret_cast<uint32_t*>(ptr);
	//	//for (int i = 0; i < sizeof(indices) / sizeof(indices[0]); i++) {
	//	//	std::cout << "Index:" << i << ": " << mappedVertices[i] << std::endl;
	//	//}
	...
	//	if (!SearchFilePath(L"SampleTexture.dds", texturePath)) {

	//		std::wcout << L"Not Found " << L"texture/house.dds" << L" Texture" << std::endl;
	//		return false;
	//	}
	...
	//	InputElement[0].Format               = DXGI_FORMAT_R32G32B32A32_FLOAT;              /* 入力のフォーマットを指定 */
	...
	//	HRESULT hr = D3DReadFileToBlob(L"VertexShader.cso", pVSBlob.GetAddressOf());
	//	if (FAILED(hr)) {
	//		return false;
	//	}
	 ...
	 ***************************************************/
	 ...
	 // 定数バッファの生成.
	 {
		 ...
		 for (auto i = 0; i < FrameCount; ++i)
		 {
			 ...
			 m_CBV[i].CBVDesc.BufferLocation = address;
			 m_CBV[i].CBVDesc.SizeInBytes = sizeof(Transform);
			 ...
			 m_CBV[i].pBuffer->Projection = DirectX::XMMatrixPerspectiveFovRH(fovY, aspect, 1.0f, 1000.0f);
		 }
	 }
	 ...
	 // パイプラインステートの生成.
	 {
		...
		descDSS.DepthEnable    = TRUE;                              /* 深度テストを行うかどうか */
		descDSS.DepthWriteMask = D3D12_DEPTH_WRITE_MASK_ALL;        /* 深度ステンシルバッファへの書き込みを有効にする */
		descDSS.DepthFunc      = D3D12_COMPARISON_FUNC_LESS_EQUAL;  /* 比較の際の関数（今回は「以下」を設定） */
		descDSS.StencilEnable  = FALSE;                             /* ステンシルテストを行うかどうか */
...
void App::Render() {
	...
	// 描画処理.
	{
		...
		m_pCmdList->SetGraphicsRootConstantBufferView(0, m_CBV[m_FrameIndex].CBVDesc.BufferLocation);
		m_pCmdList->SetGraphicsRootDescriptorTable(1, m_Texture.HandleGPU);
